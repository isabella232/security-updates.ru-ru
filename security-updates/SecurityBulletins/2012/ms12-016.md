...
Чтобы просмотреть стандартную запись об этой уязвимости в списке "Common Vulnerabilities and Exposures", см. [CVE-2012-0014](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-0014).
...
#### Временные решения для уязвимости .NET Framework, приводящей к повреждению кучи (CVE-2012-0015)
...
**Каким образом злоумышленник может воспользоваться этой уязвимостью?**
Существует три типа сценариев атак, подверженных данной уязвимости: сценарии на базе веб-браузера, сценарии на базе веб-хостинга и обход приложениями Windows .NET сценария ограничений разграничения доступа кода (CAS). Ниже приведены описания каждого из случаев.
...
**Как действует обновление?**
Обновление устраняет уязвимость, изменяя в Microsoft .NET Framework способ вычисления длины буфера.

**Была ли эта уязвимость опубликована до выпуска этого бюллетеня безопасности?**
Да. О данной уязвимости сообщалось в открытых источниках. В списке "Common Vulnerabilities and Exposures" данной уязвимости присвоен номер [CVE-2012-0015](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-0015).
...
Да. Это обновление безопасности устраняет уязвимость, которую можно использовать с помощью опубликованного концептуального кода. В списке "Common Vulnerabilities and Exposures" уязвимости, устраняемой данным обновлением, присвоен номер [CVE-2012-0015](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-0015).
...
<td style="border:1px solid black;">См. статью <a href="http://support.microsoft.com/kb/2651026">2651026 базы знаний Майкрософт</a></td>
</tr>
<tr class="odd">
...
**Установка обновления** **для** **Microsoft .NET** **Framework 2.0** с пакетом обновления 2 (SP2)
...
**Установка** **обновления** **для** **Microsoft .NET Framework 4**

Это обновление поддерживает следующие параметры установки для Microsoft .NET Framework 4.
...
**Установка обновления** **для** **Microsoft** **.NET Framework 3.5.1**

Это обновление поддерживает следующие параметры установки для Microsoft .NET Framework 3.5.1.
...
-   Йеруна Фрейтерса (Jeroen Frijters) из компании [Sumatra](http://www.sumatra.nl/) за сообщение об уязвимости неуправляемых объектов в .NET Framework (CVE-2012-0014)
...
-   Вер. 1.1 (14 февраля 2012): В раздел "Часто задаваемые вопросы об этом обновлении" добавлена запись с объяснением изменения обнаружения для обновления KB2668562 для Microsoft Silverlight 4 в случае установки на клиентах и серверах Windows для устранения проблемы установки. Это изменение касается только логики обнаружения. В файлы обновления безопасности изменений не вносилось. Пользователям, уже выполнившим успешное обновление компьютеров, не требуется выполнять никаких действий.
-   Вер. 1.2 (15 февраля 2012): Из раздела **Аннотация** исключено ошибочное указание на известные проблемы.
-   Версия 1.3 (10 июля 2012 г.): Новая редакция этого бюллетеня выпущена в связи с тем, что в нее включена информация о небольшом изменении логики обнаружения для обновления KB2633880 для Microsoft .NET Framework 2.0 с пакетом обновления 2 (SP2) с целью устранения проблемы предоставления. В файлы обновления безопасности изменений не вносилось. Пользователям, уже выполнившим успешное обновление компьютеров, не требуется выполнять никаких действий.
