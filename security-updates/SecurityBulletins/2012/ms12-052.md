---
TOCTitle: 'MS12-052'
Title: 'Бюллетень по безопасности (Майкрософт) MS12-052 - Критическое'
ms:assetid: 'ms12-052'
ms:contentKeyID: 61237592
ms:mtpsurl: 'https://technet.microsoft.com/ru-RU/library/ms12-052(v=Security.10)'
---

Security Bulletin

Бюллетень по безопасности (Майкрософт) MS12-052 - Критическое
=============================================================

Накопительное обновление для системы безопасности браузера Internet Explorer (2722913)
--------------------------------------------------------------------------------------

Дата публикации: 14 августа 2012 г. | Дата обновления: 17 июля 2013 г.
...
**Известные проблемы.** В [статье 2722913 базы знаний Майкрософт](http://support.microsoft.com/kb/2722913) описаны известные на данный момент проблемы, с которыми пользователи могут столкнуться при установке этого обновления безопасности. В этой статье также приводятся решения, рекомендованные для устранения этих проблем.
...
<td style="border:1px solid black;">
Обновление KB2699988 в бюллетене [MS12-037](http://go.microsoft.com/fwlink/?linkid=249045) заменено обновлением KB2722913
</td>
</tr>
...
<td style="border:1px solid black;">
Обновление KB2719177 в бюллетене [MS12-044](http://go.microsoft.com/fwlink/?linkid=254377) заменено обновлением KB2722913
</td>
</tr>
<tr>
<td style="border:1px solid black;">
...
Хэши SHA1 и SHA2 обновлений для системы безопасности можно использовать для проверки подлинности загруженных пакетов обновлений для системы безопасности. Сведения о хэшах, относящиеся к данному обновлению, см. в [статье 2722913 базы знаний Майкрософт](http://support.microsoft.com/kb/2722913).
...
**Какое отношение имеет данное обновление для системы безопасности к бюллетеню** **MS12-056?**
Описанная в данном бюллетене уязвимость в JavaScript, связанная с переполнением целочисленного значения и делающая возможным удаленное выполнение кода (CVE-2012-2523), также устраняется обновлением [MS12-056](http://go.microsoft.com/fwlink/?linkid=256487).

Пользователям, в системах которых установлен браузер Internet Explorer 8, следует установить описанное в бюллетене [MS12-056](http://go.microsoft.com/fwlink/?linkid=256487) обновление KB2706045, чтобы защититься от уязвимости, описанной в CVE-2012-2523.

Пользователям, в системе которых установлен браузер Internet Explorer 9, не требуется устанавливать обновление KB2706045, так как описанное в данном бюллетене обновление (KB2722913) устраняет уязвимость CVE-2012-2523 для систем с Internet Explorer 9.
...
<th style="border:1px solid black;" >
Уязвимость макета, приводящая к повреждению памяти (CVE-2012-1526)
</th>
<th style="border:1px solid black;" >
Уязвимость асинхронного доступа к пустому объекту, делающая возможным удаленное выполнение кода (CVE-2012-2521)
</th>
<th style="border:1px solid black;" >
Уязвимость повреждения таблицы виртуальных функций, делающая возможным удаленное выполнение кода (CVE-2012-2522)
</th>
<th style="border:1px solid black;" >
...
Чтобы просмотреть стандартную запись об этой уязвимости в списке "Common Vulnerabilities and Exposures", см. [CVE-2012-1526](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-1526).

#### Смягчающие факторы для уязвимости макета, приводящей к повреждению памяти (CVE-2012-1526)
...
#### Временные решения для уязвимости макета, приводящей к повреждению памяти (CVE-2012-1526)
...
#### Часто задаваемые вопросы об уязвимости макета, приводящей к повреждению памяти (CVE-2012-1526)
...
Для вредоносного действия с использованием этой уязвимости требуется, чтобы пользователь выполнил вход в систему и посетил веб-сайт. Поэтому все системы, в которых часто используется браузер Internet Explorer 6 или Internet Explorer 7, например рабочие станции или серверы терминалов, подвержены наибольшему риску использования этой уязвимости.
...
Чтобы просмотреть стандартную запись об этой уязвимости в списке "Common Vulnerabilities and Exposures", см. [CVE-2012-2521](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-2521).

#### Смягчающие факторы для уязвимости асинхронного доступа к пустому объекту, делающей возможным удаленное выполнение кода (CVE-2012-2521)
...
#### Временные решения для уязвимости асинхронного доступа к пустому объекту, делающей возможным удаленное выполнение кода (CVE-2012-2521)
...
#### Часто задаваемые вопросы об уязвимости асинхронного доступа к пустому объекту, делающей возможным удаленное выполнение кода (CVE-2012-2521)
...
<span></span>
В способе, используемом браузером Internet Explorer для доступа к удаленной поврежденной таблице виртуальных функций, существует уязвимость, делающая возможным удаленное выполнение кода. В результате память может быть повреждена так, что злоумышленник сможет выполнить произвольный код в контексте текущего пользователя.

Чтобы просмотреть стандартную запись об этой уязвимости в списке "Common Vulnerabilities and Exposures", см. [CVE-2012-2522](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-2522).

#### Смягчающие факторы для уязвимости повреждения таблицы виртуальных функций, делающей возможным удаленное выполнение кода (CVE-2012-2522)
...
#### Временные решения для уязвимости повреждения таблицы виртуальных функций, делающей возможным удаленное выполнение кода (CVE-2012-2522)
...
#### Часто задаваемые вопросы об уязвимости повреждения таблицы виртуальных функций, делающей возможным удаленное выполнение кода (CVE-2012-2522)

**Какова область воздействия этой уязвимости?**
Эта уязвимость возникает в контексте текущего пользователя и приводит к удаленному выполнению кода.

**В чем причина уязвимости?**
Когда браузер Internet Explorer пытается получить доступ к поврежденной таблице виртуальных функций, он может повредить память так, что злоумышленник сможет выполнить произвольный код в контексте текущего пользователя.
...
<span></span>
В способе, используемом Internet Explorer для вычисления размера объекта в памяти во время операции копирования, существует уязвимость, делающая возможным удаленное выполнение кода. В результате память может быть повреждена так, что злоумышленник сможет выполнить произвольный код в контексте текущего пользователя.
...
**В чем причина уязвимости?**
Данная уязвимость возникает, когда Internet Explorer неправильно вычисляет размер объекта в памяти во время операции копирования.
...
**Какие системы в первую очередь подвержены риску?**
Системы, в которых часто используется браузер Internet Explorer 8 или Internet Explorer 9, например рабочие станции или серверы терминалов, подвержены наибольшему риску использования этой уязвимости.

**Я работаю с браузером Internet Explorer для Windows Server 2003, Windows Server 2008 или Windows Server 2008 R2. Снижает ли это обновление опасность уязвимости?**
Да. По умолчанию Internet Explorer в Windows Server 2003, Windows Server 2008 и Windows Server 2008 R2 работает в ограниченном режиме, который называется [конфигурация усиленной безопасности](http://technet.microsoft.com/library/dd883248.aspx). Конфигурация усиленной безопасности Internet Explorer — это группа предварительно настроенных параметров, снижающих для пользователей и администраторов риск загрузить или выполнить на сервере особым образом созданное веб-содержимое. Это смягчающий фактор для веб-сайтов, не добавленных в зону "Надежные узлы" веб-браузера Internet Explorer.

**Как действует обновление?**
Это обновление устраняет эту уязвимость для Internet Explorer 9, изменяя способ работы браузера Internet Explorer с объектами в памяти.

**Примечание.** Данное обновление не устраняет данную уязвимость для Internet Explorer 8. Для Internet Explorer 8 доступно обновление KB2706045, описанное в [MS12-056](http://go.microsoft.com/fwlink/?linkid=256487).

**В моей системе установлен браузер Internet Explorer 8. Где найти обновление, устраняющее уязвимость CVE-2012-2523?**
Пользователи, в системах которых установлен браузер Internet Explorer 8, могут устранить уязвимость, описанную в CVE-2012-2523, установив обновление KB2706045, описанное в [MS12-056](http://go.microsoft.com/fwlink/?linkid=256487).

Пользователям, в системе которых установлен браузер Internet Explorer 9, не требуется устанавливать обновление KB2706045, так как данное обновление (KB2722913) устраняет уязвимость CVE-2012-2523 для систем с Internet Explorer 9.
...
Используйте средство <strong>Установка и удаление программ</strong> панели управления или служебную программу Spuninst.exe, которая находится в папке %Windir%\$NTUninstallKB2722913$\Spuninst.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">Веб-браузер Internet Explorer 7 для всех поддерживаемых выпусков Windows XP:<br />
Используйте компонент <strong>Установка и удаление программ</strong> панели управления или служебную программу Spuninst.exe, которая находится в папке %Windir%\ie7updates\KB2722913-IE7\spuninst.</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">Веб-браузер Internet Explorer 8 для всех поддерживаемых выпусков Windows XP:<br />
Используйте компонент <strong>Установка и удаление программ</strong> панели управления или служебную программу Spuninst.exe, которая находится в папке %Windir%\ie8updates\KB2722913-IE8\spuninst.</td>
</tr>
<tr class="odd">
...
<td style="border:1px solid black;"><strong>Сведения об удалении</strong></td>
<td style="border:1px solid black;">Веб-браузер Internet Explorer 6 для всех поддерживаемых 32-разрядных выпусков Windows Server 2003, выпусков Windows Server 2003 x64 Edition и выпусков Windows Server 2003 для платформы Itanium:<br />
Используйте компонент <strong>Установка и удаление программ</strong> панели управления или служебную программу Spuninst.exe, которая находится в папке %Windir%\$NTUninstallKB2722913$\Spuninst.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">Веб-браузер Internet Explorer 7 для всех поддерживаемых 32-разрядных выпусков Windows Server 2003, выпусков Windows Server 2003 x64 Edition и выпусков Windows Server 2003 для платформы Itanium:<br />
...
-   GWSlabs, сотрудничающих с компанией [VeriSign iDefense Labs](http://labs.idefense.com/), за сообщение об уязвимости макета, приводящей к повреждению памяти (CVE-2012-1526)  
-   Дерека Соудера (Derek Soeder), сотрудничающего в рамках программы [SecuriTeam Secure Disclosure компании Beyond Security](http://www.beyondsecurity.com/ssd.html), за сообщение об уязвимости асинхронного доступа к пустому объекту, делающей возможным удаленное выполнение кода (CVE-2012-2521)  
-   Сунь-тин Цая (Sung-ting Tsai) и Мин-Чи Пана (Ming-Chieh Pan) из компании [Trend Micro](http://www.trendmicro.com/) за сообщение об уязвимости повреждения таблицы виртуальных функций, делающей возможным удаленное выполнение кода (CVE-2012-2522)  
...
-   Вер. 1.1 (15 августа 2012 г.): Из раздела часто задаваемых вопросов удалена ошибочная запись для выпусков Windows 8 Release Preview и Windows Server 2012 Release Candidate. Выпуски Windows 8 Release Preview и Windows Server 2012 Release Candidate не подвержены уязвимостям, описанным в данном бюллетене.  
-   Вер. 1.2 (17 июля 2013 г.): В подраздел "Известные проблемы" раздела "Аннотация" добавлена ссылка на статью 2722913 базы знаний Майкрософт.
