---
TOCTitle: 'MS13-008'
Title: 'Бюллетень по безопасности (Майкрософт) MS13-008 - Критическое'
ms:assetid: 'ms13-008'
ms:contentKeyID: 61237631
ms:mtpsurl: 'https://technet.microsoft.com/ru-RU/library/ms13-008(v=Security.10)'
---

Security Bulletin

Бюллетень по безопасности (Майкрософт) MS13-008 - Критическое
=============================================================

Обновление для системы безопасности Internet Explorer (2799329)
---------------------------------------------------------------
...
Это обновление для системы безопасности имеет уровень "критический" для браузеров Internet Explorer 6, Internet Explorer 7 и Internet Explorer 8 на клиентских системах Windows, и уровень "средний" для браузеров Internet Explorer 6, Internet Explorer 7 и Internet Explorer 8 на серверах Windows. Браузеры Internet Explorer 9 и Internet Explorer 10 не подвержены этой уязвимости. Дополнительные сведения см. в подразделе **Подвержены и не подвержены уязвимости** далее в этом разделе.
...
Кроме того, это обновление системы безопасности устраняет уязвимость, впервые описанную в [выпуске 2794220 советов по безопасности (Майкрософт)](http://technet.microsoft.com/security/advisory/2794220).
...
<span></span>
**Является ли это обновление MS13-008 накопительным** **обновлением для системы безопасности браузера Internet Explorer?**
Нет. Это обновление MS13-008 только устраняет уязвимость, описанную в данном бюллетене.

**Нужно ли мне устанавливать последнее** **накопительное обновление для системы безопасности браузера Internet Explorer** **— MS12-077?**
Да. Во всех случаях обновление MS13-008 защищает пользователей от уязвимости, описанной в данном бюллетене. Однако, пользователи, не установившие последнее накопительное обновление для системы безопасности браузера Internet Explorer, могут столкнуться с проблемами совместимости после установки обновления MS13-008.

Чтобы избежать проблем с совместимостью, пользователи должны убедиться, что установили последнее накопительное обновление для системы безопасности браузера — [MS12-077](http://go.microsoft.com/fwlink/?linkid=268393).

**Если применено** **автоматическое средство Microsoft Fix it** **для** **Internet Explorer, предоставленное в выпуске** **2794220 советов по безопасности (Майкрософт),** **нужно ли отменять это временное решение перед применением данного обновления?**
Пользователям, которые применили средство **Microsoft** **Fix** **it** "MSHTML Shim Workaround" (Временное решение с использованием HTML-модификатора MS), предоставленное в выпуске [Microsoft Security Advisory 2794220](http://technet.microsoft.com/security/advisory/2794220), не требуется отменять изменения, внесенные с помощью **Microsoft Fix it**, перед применением данного обновления.
...
Хэши SHA1 и SHA2 обновлений для системы безопасности можно использовать для проверки подлинности загруженных пакетов обновлений для системы безопасности. Сведения о хэшах, относящиеся к данному обновлению, см. в [статье 2799329 базы знаний Майкрософт](https://support.microsoft.com/kb/2799329).
...
<th style="border:1px solid black;" >
Уязвимость, связанная с использованием Internet Explorer после освобождения — CVE-2012-4792
</th>
<th colspan="2">
Общий уровень серьезности
</th>
...
Чтобы просмотреть стандартную запись об этой уязвимости в списке "Common Vulnerabilities and Exposures", см. [CVE-2012-4792](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-4792).

#### Смягчающие факторы для уязвимости, связанной с использованием Internet Explorer после освобождения — CVE-2012-4792
...
#### Временные решения для уязвимости, связанной с использованием Internet Explorer после освобождения — CVE-2012-4792

К временным решениям относятся изменения параметров или конфигурации, не приводящие к полному устранению уязвимости, но позволяющие блокировать известные направления атак до того, как будет установлено обновление. Корпорация Майкрософт проверила представленные ниже временные решения и сообщила, снижают ли они функциональные возможности.

-   **Установите решение Microsoft Fix it "MSHTML Shim Workaround" (Временное решение с использованием HTML-модификатора MS), которое предотвращает использование этой уязвимости**

    В [статье 2799329 базы знаний Майкрософт](http://support.microsoft.com/kb/2799329) приведены инструкции по использованию автоматизированного средства **Microsoft Fix it** для применения/отмены этого временного решения.
...
#### Часто задаваемые вопросы об уязвимости, связанной с использованием Internet Explorer после освобождения — CVE-2012-4792
...
Злоумышленник может создать специальный веб-сайт, предназначенный для использования данной уязвимости через браузер Internet Explorer. Кроме того, он может воспользоваться уязвимостью через веб-сайты, которые уже подверглись атаке, и веб-сайты, которые принимают или размещают сведения или рекламу. Веб-сайты могут включать специальное содержимое, использующее данную уязвимость. Однако в любом случае злоумышленник не может заставить пользователей просмотреть контролируемое злоумышленником содержимое. Вместо этого злоумышленнику придется убедить пользователей совершить какое-либо действие: например, перейти по ссылке на веб-сайт в сообщении электронной почты или программы обмена мгновенными сообщениями или открыть вложение сообщения электронной почты.
...
Да. О данной уязвимости сообщалось в открытых источниках. Данной уязвимости присвоен номер [CVE-2012-4792](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-4792) в списке "Common Vulnerability and Exposures". Впервые данная уязвимость была описана в [выпуске 2794220 советов по безопасности (Майкрософт)](http://technet.microsoft.com/security/advisory/2794220).

**Получала ли корпорация Майкрософт к моменту выпуска этого бюллетеня по безопасности какие-либо сведения о том, что уязвимость была использована злоумышленниками?**
Да. Специалистам Майкрософт известно о направленных атаках с целью использования этой уязвимости через Internet Explorer 8.
...
<td style="border:1px solid black;">Веб-браузер Internet Explorer 6 для всех поддерживаемых выпусков Windows XP:<br />
Используйте средство <strong>Установка и удаление программ</strong> на панели управления или служебную программу Spuninst.exe, которая находится в папке %Windir%\$NTUninstallKB2799329$\Spuninst.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">Веб-браузер Internet Explorer 7 для всех поддерживаемых выпусков Windows XP:<br />
Используйте средство <strong>Установка и удаление программ</strong> на панели управления или служебную программу Spuninst.exe, которая находится в папке %Windir%\ie7updates\KB2799329-IE7\spuninst.</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">Веб-браузер Internet Explorer 8 для всех поддерживаемых выпусков Windows XP:<br />
Используйте средство <strong>Установка и удаление программ</strong> на панели управления или служебную программу Spuninst.exe, которая находится в папке %Windir%\ie8updates\KB2799329-IE8\spuninst.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>Сведения о файлах</strong></td>
<td style="border:1px solid black;">См. <a href="http://support.microsoft.com/kb/2799329">статью 2799329 базы знаний Майкрософт</a></td>
</tr>
<tr class="even">
...
-   Компанию [Exodus Intelligence](https://www.exodusintel.com) за совместную работу над устранением уязвимости, связанной с использованием Internet Explorer после освобождения (CVE-2012-4792)
...
-   Версия 1.0 (14 января 2013 г.): Бюллетень опубликован.
