...
<sup>[1]</sup>Этот загружаемый файл обновляет Microsoft Silverlight 2 до более новой версии Microsoft Silverlight, которая не подвержена этим уязвимостям.
...
Это обновление для системы безопасности устанавливает версию 3.0.41130.0. Если у вас установлена эта или более поздняя версия, ваша система не подвержена уязвимости.
...
#### Часто задаваемые вопросы об уязвимости в Microsoft Silverlight, приводящей к повреждению памяти (CVE-2010-0019)

**Какова область воздействия этой уязвимости?**  
Если в системе установлена платформа Microsoft Silverlight, эта уязвимость делает возможным удаленное выполнение кода от имени вошедшего в систему пользователя.

**В чем причина уязвимости?**  
Эта уязвимость вызвана способом обработки указателей платформой Silverlight. Когда платформа Silverlight проводит синтаксический анализ специально созданного кода Silverlight, возможно повреждение памяти, в результате чего платформа может перестать отвечать на запросы или станет возможным удаленное выполнение кода.
...
Злоумышленник может воспользоваться этой уязвимостью, только если вошедший в систему пользователь просмотрит веб-страницу в браузере, который способен создавать экземпляры приложений Silverlight. Таким образом, в первую очередь риску подвергаются любые системы с установленной платформой Microsoft Silverlight (как Microsoft Windows, так и Apple Mac OS). Поэтому риску использования данной уязвимости наиболее подвержены системы, в которых часто используется такой браузер, например рабочие станции или серверы терминалов. Риск для серверов повышается, если администраторы разрешают пользователям переходить на веб-сайты и читать сообщения электронной почты на сервере; тем не менее, общепринятой практикой настоятельно рекомендуется не предоставлять пользователям таких полномочий.
...
Это обновления исправляет способ обработки указателей платформой Microsoft Silverlight.
...
<span></span>
В платформе Microsoft .NET Framework существует уязвимость, которая делает возможным удаленное выполнение кода. Воспользовавшееся ею специально созданное приложение Microsoft .NET или специально созданное приложение Silverlight может получать доступ к памяти, что делает возможным выполнение произвольного кода.

В списке "Common Vulnerabilities and Exposures" данной уязвимости присвоен номер [CVE-2010-1898](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-1898).

#### Смягчающие факторы для уязвимости в Microsoft Silverlight и Microsoft .NET Framework CLR, связанной с делегатами виртуальных методов (CVE-2010-1898)
...
-   Платформа Microsoft Silverlight 4, текущая версия этой платформы, не подвержена данной уязвимости. По умолчанию платформа Silverlight периодически проверяет наличие на веб-сайте Майкрософт обновлений с последними функциями и усовершенствованиями. Более новые версии автоматически загружаются и устанавливаются на компьютере. В большинстве систем версия Silverlight 4 уже установлена. Подробности см. на сайте [средства обновления Microsoft Silverlight](http://www.microsoft.com/silverlight/resources/documentation/updater.aspx).

#### Временные решения для уязвимости в Microsoft Silverlight и Microsoft .NET Framework CLR, связанной с делегатами виртуальных методов (CVE-2010-1898)
...
-   **Отключение браузерных приложений XAML в Internet Explorer**
    ...
    1.  В Internet Explorer откройте меню **Сервис** и выберите пункт **Свойства обозревателя**.
    2.  На вкладке **Безопасность** щелкните зону **Интернет**, а затем нажмите кнопку **Другой**.
    ...
    **Отмена временного решения.**
    ...
    1.  В Internet Explorer откройте меню **Сервис** и выберите пункт **Свойства обозревателя**.
    2.  Перейдите на вкладку **Безопасность**, нажмите кнопку **Выбрать уровень безопасности по умолчанию для всех зон**, а затем кнопку **OK**.
...
-   **Временный запрет на выполнение элемента управления ActiveX платформы Microsoft Silverlight в Internet Explorer в ОС Windows XP с пакетом обновления 2 (SP2) или более поздней версии**
    ...
    Чтобы защитить систему от этих уязвимостей, временно отключите запуск элемента управления ActiveX для платформы Silverlight в веб-браузере Internet Explorer. Отключить элемент управления ActiveX в Internet Explorer можно при помощи функции «Управление надстройками».

    1.  В браузере Internet Explorer откройте меню **Сервис** и выберите пункт **Управление надстройками**.
    2.  Выберите пункт **Microsoft Silverlight**.
...
-   **Обновление платформы Microsoft Silverlight до последней версии**

    Версия Silverlight 4 не подвержена этой уязвимости. Вместо установки обновления для системы безопасности Silverlight пользователи могут обновить платформу Silverlight до последней версии. Дополнительные сведения см. на веб-сайте [Microsoft Silverlight](http://www.microsoft.com/silverlight/).

#### Часто задаваемые вопросы об уязвимости в Microsoft Silverlight и Microsoft .NET Framework CLR, связанной с делегатами виртуальных методов (CVE-2010-1898)
...
**В чем причина уязвимости?**  
Эта уязвимость вызвана методом обработки делегатов виртуальных методов средой CLR платформы Microsoft .NET.
...
| Microsoft Silverlight 3 в ОС Microsoft Windows                                                                             | Да         |
| Microsoft Silverlight 3 в ОС Mac                                                                                           | Нет        |
...
| Microsoft Silverlight 3 в ОС Microsoft Windows                                                                             | Нет     | Нет                        | Нет                        | Да                          |
| Microsoft Silverlight 3 в ОС Mac                                                                                           | Нет     | Нет                        | Нет                        | Нет                         |
...
<td style="border:1px solid black;">См. статью <a href="http://support.microsoft.com/kb/2265906">2265906 базы знаний Майкрософт</a></td>
</tr>
<tr class="odd">
...
**Установка обновления для Microsoft .NET Framework 3.5 (KB983582) и Microsoft .NET Framework 2.0 с пакетом обновления 2 (SP2) и Microsoft .NET Framework 3.5 с пакетом обновления 1 (SP1) (KB983583)**
...
**Замечание об обновлении для Microsoft .NET Framework 2.0 с пакетом обновления 2 (SP2) (KB983583)**. При установке этого обновления для системы безопасности установщик проверяет, проводилось ли ранее обновление данного продукта либо его компонентов с помощью исправления Майкрософт.
...
**Удаление обновления для Microsoft .NET Framework 3.5 (KB983582) и Microsoft .NET Framework 2.0 с пакетом обновления 2 (SP2) и Microsoft .NET Framework 3.5 с пакетом обновления 1 (SP1) (KB983583)**
...
<tr class="odd">
<td style="border:1px solid black;"><strong>Проверка раздела реестра</strong></td>
<td style="border:1px solid black;">Для Microsoft .NET Framework 3.5:<br />
HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Updates\Microsoft .NET Framework 2.0\Service Pack 1\SP1\KB983582<br />
&quot;ThisVersionInstalled&quot; = &quot;Y&quot;</td>
</tr>
...
**Установка обновления для Microsoft .NET Framework 2.0 с пакетом обновления 1 (SP1) и Microsoft .NET Framework 3.5 (KB983587) и Microsoft .NET Framework 2.0 с пакетом обновления 2 (SP2) и Microsoft .NET Framework 3.5 с пакетом обновления 1 (SP1) (KB983588 и KB983589)**
...
-   Карстен Бук (Carsten Book) из за сообщение об уязвимости в Microsoft Silverlight, приводящей к повреждению памяти (CVE-2010-0019)  
-   Имона Нербонна ([Eamon Nerbonne](http://eamon.nerbonne.org)) за сообщение об уязвимости в Microsoft Silverlight и Microsoft .NET Framework CLR, связанной с делегатами виртуальных методов (CVE-2010-1898)
...
-   Вер. 1.1 (11 августа 2010): В подразделе "Известные проблемы" раздела "Аннотация" добавлена ссылка на статью 2265906 базы знаний Майкрософт. Кроме того, исправлены записи для Microsoft Silverlight в таблице "Не подвержены уязвимости" и в разделе "Временные решения для уязвимости в Microsoft Silverlight, приводящей к повреждению памяти (CVE-2010-0019)".  
-   Вер. 1.2 (29 сентября 2010 г.): Добавлено значение параметра "Compatibility Flags" к временному решению "Временное отключение элемента управления ActiveX для платформы Microsoft Silverlight в браузере Internet Explorer (метод №1)".
